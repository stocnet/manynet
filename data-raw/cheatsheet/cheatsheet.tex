...
\newcommand{\gundirected}{\glyph{
  ...
  \path[gtie] (a)--(b);}}
% diffusion S-curve
\newcommand{\gdiffusion}{\begin{tikzpicture}[baseline=-2pt, scale=0.4]
...
\newcommand{\gwaves}{\begin{tikzpicture}[baseline=-2pt, scale=0.34]
  % frame 1
...
loops \& layers: \code|to_simplex()|, \code|to_uniplex(net, layer)|,
\code|to_flat(net, rule)|
...
\item \code|to_layers()| / \code|from_layers()|: split into layers or
join networks as layers
\end{itemize}
\end{mnbox}
