...
list_functions <- function(string){
  paste0("`", paste(paste0("[", ls("package:manynet")[grepl(string, ls("package:manynet"))], "()](https://stocnet.github.io/manynet/reference/", ls("package:manynet")[grepl(string, ls("package:manynet"))], ".html)"), collapse = "`, `"), "`")
}
...
By contrast, `{manynet}` offers _many_ tools that work on _many_ (if not most) types and kinds of networks.
It helps researchers **make**, **modify**, **mark**, and **measure** networks,
as well as identify nodes' **motifs** and **memberships**.
`{manynet}` works with one-mode and two-mode (bipartite) networks,
directed and undirected, weighted and unweighted, signed, multiplex, and dynamic (longitudinal) networks.
All functions operate consistently on matrices, edge lists, and `{igraph}`, `{network}`, and `{tidygraph}` objects.

For graph drawing see [`{autograph}`](https://stocnet.github.io/autograph/),
for network analysis see [`{netrics}`](https://stocnet.github.io/netrics/)
...
- [Making](#making)
    ...
    - [Simulating processes on networks](#simulating-processes-on-networks)
...
- [Marking](#marking)
    - [Marking networks](#marking-networks)
    - [Marking nodes and ties](#marking-nodes-and-ties)
- [Measuring](#measuring)
    - [Measuring networks](#measuring-networks)
    - [Measuring nodes and ties](#measuring-nodes-and-ties)
...
For example, `n = 15` will create a one-mode network of 15 nodes,
...
#### Simulating processes on networks
...
network-level and node-level diffusion measures in `{manynet}`.
...
ison_southern_women %>%
  ...
  filter_nodes(node_is_isolate(ison_southern_women)) %>%
  select_nodes(name)
```

Available verbs for manipulating **nodes**: `r list_functions("^(mutate|filter|select|rename|arrange|bind|add|delete|join)_nodes$")`

Available verbs for manipulating **ties**: `r list_functions("^(mutate|filter|select|rename|arrange|bind|add|delete|join)_ties$")`
...
`{manynet}` makes it easy to mark (identify) properties of networks, nodes, and ties
using logical tests that return `TRUE` or `FALSE`.

#### Marking networks

The `is_*()` functions test whether a network has a particular property.
This is useful for conditional analysis, validation, and understanding what you are working with:

```{r mark-example, eval = FALSE}
...
Available network marks include: `r list_functions("^is_")`

#### Marking nodes and ties

The `node_is_*()` functions return a logical vector identifying which nodes
have a particular property,
and the `tie_is_*()` functions do the same for ties.
These are especially useful for filtering or visualising subsets:

```{r nodemark-example, eval = FALSE}
ison_southern_women %>%
  mutate_nodes(is_isolate = node_is_isolate(ison_southern_women))
```

Available node marks: `r list_functions("^node_is_")`

Available tie marks: `r list_functions("^tie_is_")`

## Measuring

`{manynet}` offers an extensive suite of measures at both the network and node/tie level.
All measures work consistently across network types and input classes.

#### Measuring networks

The `net_*()` functions return a single numeric value summarising a property of the whole network:

```{r netmeasure-example, eval = FALSE}
net_density(ison_southern_women)
net_cohesion(ison_southern_women)
net_transitivity(ison_southern_women)
```

Key families of network measures include:

- **Dimensions**: `net_nodes()`, `net_ties()`, `net_dims()`, `net_density()`
- **Centralization**: `net_degree()`, `net_betweenness()`, `net_closeness()`, `net_eigenvector()`
- **Connectivity**: `net_components()`, `net_cohesion()`, `net_adhesion()`, `net_diameter()`, `net_length()`
- **Topology**: `net_transitivity()`, `net_reciprocity()`, `net_balance()`, `net_modularity()`
- **Heterogeneity**: `net_richness()`, `net_diversity()`, `net_homophily()`, `net_assortativity()`
- **Innovation**: `net_smallworld()`, `net_scalefree()`, `net_independence()`
- **Diffusion**: `net_infection_total()`, `net_infection_peak()`, `net_recovery()`, `net_reproduction()`

#### Measuring nodes and ties

The `node_*()` functions return a numeric vector the length of the nodes in the network.
These centrality and other measures can be used directly in analysis or added as attributes:

```{r nodemeasure-example, eval = FALSE}
ison_southern_women %>%
  mutate_nodes(degree = node_degree(ison_southern_women),
               betweenness = node_betweenness(ison_southern_women))
```

Key families of node measures include:

- **Centrality**: `node_degree()`, `node_betweenness()`, `node_closeness()`, `node_eigenvector()`, `node_pagerank()`
- **Brokerage**: `node_constraint()`, `node_bridges()`, `node_brokering()`
- **Local structure**: `node_transitivity()`, `node_reciprocity()`, `node_redundancy()`
- **Position**: `node_coreness()`, `node_eccentricity()`, `node_reach()`
- **Diffusion**: `node_adoption_time()`, `node_thresholds()`, `node_exposure()`

Tie measures (e.g. `tie_betweenness()`, `tie_closeness()`) work analogously for edges.

## Motifs and Memberships

`{manynet}` provides tools for identifying local substructure patterns (motifs)
and assigning nodes to groups (memberships) such as communities or equivalence classes.

#### Motifs

Motif functions return census vectors counting how each node participates in
particular subgraph configurations:

```{r motif-example, eval = FALSE}
node_by_triad(ison_adolescents)
node_by_quad(ison_southern_women)
```

Available motif censuses: `r list_functions("^node_by_")`

#### Memberships

Membership functions assign nodes to groups based on community detection algorithms
or structural equivalence methods.
These return a vector of membership assignments:

```{r member-example, eval = FALSE}
node_in_louvain(ison_adolescents)
node_in_walktrap(ison_adolescents)
node_in_structural(ison_adolescents)
```

Available membership algorithms: `r list_functions("^node_in_")`
...
but has also devolved its visualisation capabilities to its sibling package, `{autograph}`,
and its network analytic capabilities to its sibling package, `{netrics}`.
`{migraph}` continues to offer modelling functions that builds upon
