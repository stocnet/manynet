---
title: "Very short introduction to R"
...
```{printing-results, exercise = TRUE}
...
```{Print-hello-world, exercise = TRUE}
Print("Hello World")
```
...
```{df-call-var, exercise = TRUE}
mydf$Surname
```

This can be very handy when "subsetting" the data:

```{df-subset, exercise = TRUE}
...
# 2. Send your R script in which you created this matrix to the TA
