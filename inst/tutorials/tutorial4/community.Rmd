...
In this tutorial, we're going to consider various ways in which 'groupiness'
might be examined.
...
<dfn title='A measure of how often nodes in a directed network are mutually linked.'>reciprocity</dfn>
in the task network.
...
And let's calculate 
<dfn title='Triadic closure is where if the connections A-B and A-C exist among three nodes, there is a tendency for B-C also to be formed.'>transitivity</dfn> 
in the task network.
...
<dfn title='A component is a connected subgraph not part of a larger connected subgraph.'>components</dfn> 
there are.
...
<dfn title='An undirected network is one in which the edges do not have orientations.'>undirected</dfn>.
...
about the links between political blogs in the 2004 election.

```{r graphirpsblogs, warning=FALSE, fig.width=9, exercise=TRUE}
graphr(irps_blogs)
```

This network probably has a lot of components, not least because there are so many isolates.
Even if we look at weak components, we see that there are a lot of components.

```{r blogcomp, exercise = TRUE}
net_components(irps_blogs)
net_components(to_undirected(irps_blogs))
```

### Giant component

However, a lot of these components are actually isolates.
We can calculate the number of isolates by simply summing `node_is_isolate()`.

```{r blogisolates, exercise = TRUE}
sum(node_is_isolate(irps_blogs))
```

Wow, it looks like most of the (weak) components are due to isolates!
So how do we concentrate on the main component of this network?
Well, the main/largest component in a network is called the
<dfn title='The giant component is the component that includes the most nodes in the network'>giant component</dfn>.

```{r blogtogiant, exercise=TRUE, warning=FALSE, fig.width=9}
blogs <- irps_blogs %>% to_giant()
