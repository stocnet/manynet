...
graphs(play_diffusion(rg, seeds = 1:4), node_color = "Infected", layout = "circle") +
  ggplot2::guides(colour = guide_legend(""))
```
...
$R_0$, or the attribute's reproductive number, is calculated as: $R_0 = \min\left(\frac{T}{1/IL}, \bar{k}\right)$
It is measured as the transmissibility (proportion of susceptible nodes that are infected at each time period) over the average infection length (average length of time nodes remain infected).  
...
ceiling(network_immunity(rd_diff) * network_nodes(rd))
```
