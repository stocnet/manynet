...
In this tutorial, we're going to look at several contagion or diffusion processes 
...
```{r clattice, exercise = TRUE, purl = FALSE}
...
```{r lat_diff, exercise = TRUE, exercise.setup = "clattice", purl = FALSE}
lat_diff <- play_diffusion(lat)
```

We can investigate the resulting object in a few ways.
...
```{r latticedres, exercise = TRUE, exercise.setup = "lat_diff", purl = FALSE}
lat_diff
summary(lat_diff)
```
...
before that.
To plot only 'where the action is', use the argument `all = FALSE`.
...
Play these same diffusions again, this time nesting the call within `net_complete_infection()`.

```{r completeinfection, exercise = TRUE, purl = FALSE}
net_complete_infection(play_diffusion(create_ring(32, width = 2)))
net_complete_infection(play_diffusion(generate_random(32, 0.15)))
net_complete_infection(play_diffusion(generate_scalefree(32, 0.025)))
net_complete_infection(play_diffusion(generate_smallworld(32, 0.025)))
```
...
question("Does the structure of the network matter for whether and when a diffusion process completes?",
  answer("No", message = "Take a closer look at the `net_complete_infection()` results. Are they all the same?"),
...
graphr(us_diff)
grapht(us_diff)
net_complete_infection(us_diff)
```
...
illustrate the impact of thresholds on network diffusion.
...
```{r complex, exercise = TRUE, exercise.setup = "clattice", purl = FALSE}
...
plot(play_diffusion(generate_scalefree(32, 0.025), seeds = 1:8, thresholds = 2))
```
...
```{r lotr-resist, exercise=TRUE, purl = FALSE}
