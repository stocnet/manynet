...
write______(fict_lotr, "_____/lotredges.xlsx")
write______(fict_lotr, "_____/lotrnodes.xlsx")
```
  
```{r exportdata-solution, purl=FALSE}
write_edgelist(fict_lotr, "data/lotredges.xlsx")
write_nodelist(fict_lotr, "data/lotrnodes.xlsx")
```
...
Let's see what is there first.
Because the data in the `{manynet}` package are igraph objects at their core,
we can query (and manipulate) the object using the same `[` and `[[` operators.
...
removing the tie from node 1 to node 2 also removes the reciprocal tie 
...
- `to_directed()` replaces undirected ties with directed ties (arcs) or, if already directed, does nothing
- `to_redirected()` swaps the direction of directed ties (arcs) or, if undirected, does nothing
