...
#'   `test_distribution()` takes two diff_model objects and tests whether their
#'   infection curves might come from the same distribution.
#'   The null hypothesis is that they are drawn from the same distribution,
#'   and thus a p.value less than 0.05 (or other convention) might indicate
#'   that they are significantly different processes.
#'   The p.value is computed through Monte Carlo simulation.
...
#'   `test_fit()` takes a single diff_model object,
