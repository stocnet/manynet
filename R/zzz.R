...
manynet_console_theme <- function(){
  ...
  list(h1 = list(`margin-top` = 1, `margin-bottom` = 0, color = "#199D77", 
       ...
       `.alert-success` = list(before = function() paste0(col_mnet_green(symbol$tick), " ")), 
       `.alert-info` = list(before = function() paste0(col_mnet_blue(symbol$info), " ")), 
       `.alert-start` = list(before = function() paste0(symbol$arrow_right, " ")), 
